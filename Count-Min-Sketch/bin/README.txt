1. Run Attack/IPv4Universe.java --> iterate through the whole IPv4 Universe making queries to the CMS. IP addresses with a count higher than a threshold set, are saved in a file.
...
2. Run Attack/Statistics.java --> compare counts obtained in previous step with the true ones. Calculates positive concentration of IP addresses with the same number of count obtained by the attacker. The result is saved on a file.
